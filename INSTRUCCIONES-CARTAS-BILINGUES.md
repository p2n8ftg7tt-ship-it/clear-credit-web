...
## Teléfono, dirección y datos personales del reporte (especificación 005)

Estas reglas también viven en `cartas-bilingues.js` y se prueban con `node --test tests/*.test.js`
(no uses `node --test tests/`: en Node 24 falla antes de correr nada).

- **Teléfono con guiones.** En todo el analizador el teléfono es `XXX-XXX-XXXX` (por ejemplo
  `540-555-0142`): en el campo mientras la persona escribe, pega o autocompleta; en la lista de
  datos detectados; y en las tres cartas, en español y en inglés. Tiene que tener 10 dígitos; un
  `1` al inicio se descarta solo cuando hay 11 dígitos. Si no son 10 dígitos, la carta no se arma y
  se le pide un teléfono de 10 dígitos. Nunca se inventa un código de área. Las funciones son
  `telefonoEscribiendo`, `formatearTelefono` y `telefonoValido`. El campo usa `maxlength="20"` y
  no 12 a propósito: el navegador recorta lo que se pega antes de que el formato corra, y así se
  perderían dígitos.
- **La dirección sale una sola vez en «Mi información correcta».** El bloque de la carta de
  identidad tiene 7 líneas: título, nombre legal, nombre(s), primer apellido, segundo apellido,
  **una** línea de dirección actual y teléfono. Ya no se repiten calle, ciudad, estado ni código
  postal por separado. La dirección se arma una sola vez en `remitente()`.
- **Muchos nombres, direcciones y teléfonos.** Cada tarjeta de identidad muestra **todos** los
  datos detectados, agrupados en «Nombres», «Direcciones» y «Teléfonos» con su conteo y un botón
  «Marcar todos / Quitar todos» por grupo. Nada viene marcado. La carta lista exactamente lo que
  la persona marcó, en el orden nombres → direcciones → teléfonos. Qué tarjetas se ofrecen sigue
  igual (2 o más de un tipo) y lo decide `tiposDeTarjeta`; agrupar sin repetidos lo hace
  `agruparDetectados`; el orden de la carta, `ordenarDisputados`.
- **Decisiones que quedan por defecto** (se pueden cambiar): la persona escribe **un** nombre, **una**
  dirección y **un** teléfono correctos (no varios); y los bloques «MI INFORMACIÓN» de las cartas
  al buró y a la agencia de cobranza siguen repitiendo los datos del remitente.
- Detalle y criterios de aceptación: `specs/005-credit-letter-fixes/`.
