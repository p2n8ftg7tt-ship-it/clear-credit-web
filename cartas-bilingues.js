...
(function () {
  ...
  function cartaIdentidad(datos) {
    ...
    var valores = Array.isArray(datos.valoresDisputados) ? datos.valoresDisputados : [];
    return [
      bloque('remitente',
      ...
      bloque('disputados',
        ['INFORMACIÓN QUE DISPUTO:'].concat(valores.map(function (v) { return '- ' + texto(v.type) + ': ' + texto(v.value); })),
        ['INFORMATION I AM DISPUTING:'].concat(valores.map(function (v) { return '- ' + (ETIQUETAS_TIPO_DATO[texto(v.type)] || TIPO_DATO_DESCONOCIDO) + ': ' + texto(v.value); }))),
      bloque('correcta',
        ...
        ['MY CORRECT INFORMATION:',
          ...
          'Street and number: ' + r.street,
          'City: ' + r.city,
          'State: ' + r.state,
          'ZIP code: ' + r.postalCode,
